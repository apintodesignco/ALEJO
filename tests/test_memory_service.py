...
import pytest
from alejo.services.memory_service import MemoryRecord, MemoryService
    ...
    test_data = [
        ...
        },
        {
            ...
            "context": {
                ...
                "project": {"name": "ALEJO", "phase": "development"},
            },
            "importance": 0.8,
        },
        {
            ...
            "context": {
                ...
                "project": {"name": "ALEJO", "phase": "review"},
            },
            "importance": 0.6,
        },
    ]
...
async def test_partial_context_match(memory_service, sample_memories):
    ...
    context = {"project": {"name": "ALEJO"}}
    results = await memory_service.retrieve_memory(context=context)
    assert len(results) == 2
    assert all(
        "project" in r["context"] and r["context"]["project"]["name"] == "ALEJO"
        for r in results
    )
...
async def test_no_matching_context(memory_service, sample_memories):
    ...
    context = {"location": "nonexistent", "participants": ["unknown"]}
    results = await memory_service.retrieve_memory(context=context)
