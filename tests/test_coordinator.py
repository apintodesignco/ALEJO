...
class TestCoordinator:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or str(
            self.project_root / "tests" / "integration" / "test_config.json"
        )
    ...
    def setup_logging(self):
        ...
        log_file = (
            self.report_dir
            / f"alejo_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    async def run_all_tests(self):
        ...
        for stage_name, stage_func in test_stages.items():
            try:
                ...
                logger.error(
                    f"{stage_name.capitalize()} tests failed to execute: {e}",
                    exc_info=True,
                )
                results[stage_name] = {
        ...
        all_results = {
            "summary": {
                ...
                "unit_tests": results.get("unit", {}).get(
                    "summary", {"status": "NOT_RUN"}
                ),
                "component_tests": results.get("component", {}).get(
                    "summary", {"status": "NOT_RUN"}
                ),
                "integration_tests": results.get("integration", {}).get(
                    "summary", {"status": "NOT_RUN"}
                ),
                "system_tests": results.get("system", {}).get(
                    "summary", {"status": "NOT_RUN"}
                ),
            },
            "details": {
    ...
    async def run_system_tests(self):
        ...
        # Test categories
        categories = ["end_to_end", "performance", "stress", "usability"]
    ...
    def generate_report(self, results):
        ...
        report_file = (
            self.report_dir
            / f"alejo_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
...
async def main():
    ...
    print(
        f"Pass rate: {results['summary']['component_tests']['success_rate']*100:.1f}%"
    )
    ...
    print(
        f"Pass rate: {results['summary']['integration_tests']['success_rate']*100:.1f}%"
    )
    ...
    if results["summary"]["system_tests"]["total"] > 0:
        print(
            f"Pass rate: {results['summary']['system_tests']['success_rate']*100:.1f}%"
        )
