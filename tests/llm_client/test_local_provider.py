...
def local_provider(monkeypatch):
    ...
    monkeypatch.setattr("alejo.llm_client.local_provider.ModelManager", MockModelManager)
    
    # Mock the LocalLLMClient class
    monkeypatch.setattr("alejo.llm_client.local_provider.LocalLLMClient", MockLocalLLMClient)
...
async def test_error_handling(local_provider):
    ...
    with patch.object(local_provider, "_load_model_for_task", side_effect=Exception("Test error")):
