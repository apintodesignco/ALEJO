...
def lora_manager(event_bus, temp_dir):
    ...
    return LoRAManager(
        event_bus=event_bus, base_model_path=base_model_path, lora_dir=lora_dir
    )
...
async def test_list_available_adapters(lora_manager):
    ...
    await lora_manager.create_adapter(
        name="adapter1", description="Adapter 1", task_type="task1"
    )

    await lora_manager.create_adapter(
        name="adapter2", description="Adapter 2", task_type="task2"
    )
...
async def test_delete_adapter(lora_manager):
    ...
    # Create an adapter
    await lora_manager.create_adapter(
        name="adapter_to_delete", description="Adapter to delete", task_type="task"
    )
...
async def test_benchmark_adapter(lora_manager):
    ...
    metadata_path = os.path.join(
        lora_manager.lora_dir, "adapter_to_benchmark", "metadata.json"
    )
    with open(metadata_path, "r") as f:
...
async def test_task_detection(lora_manager, event_bus):
    ...
    message_event = {"message": message, "message_id": "123"}
...
async def test_singleton_instance():
    ...
    try:
        ...
        manager1 = get_lora_manager(
            event_bus=event_bus, base_model_path=base_model_path
        )
