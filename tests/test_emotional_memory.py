...
from alejo.cognitive.memory.models import EmotionalMemoryItem, EmotionalState
...
class TestEmotionalMemory:
    ...
    async def test_05_perception_event_handling(self, event_bus):
        ...
        event = Event(
            ...
            source="vision",
            payload={
                "emotion": {"valence": -0.6, "arousal": 0.7, "dominance": -0.2},
    ...
    async def test_06_emotional_state_intensity(self, event_bus):
        ...
        await memory.update_emotional_state(
            valence=1.0, arousal=1.0, dominance=1.0, source="test"
        )
        ...
        await memory.update_emotional_state(
            valence=0.5, arousal=0.5, dominance=0.5, source="test"
        )
    ...
    async def test_07_invalid_emotional_values(self, event_bus):
        ...
        with pytest.raises(ValueError):
            await memory.update_emotional_state(
                valence=1.5, arousal=0.5, dominance=0.5, source="test"  # Invalid: > 1
            )

        with pytest.raises(ValueError):
            await memory.update_emotional_state(
                valence=0.5, arousal=-1.5, dominance=0.5, source="test"  # Invalid: < -1
            )
