...
from alejo.utils.exceptions import (
    ConnectionError, MemoryError, TimeoutError,
    NetworkError, PermissionError, LLMServiceError
)
...
class TestErrorTracker(unittest.TestCase):
    ...
    @patch('alejo.utils.error_handling.ErrorTracker._load_error_counts', return_value={})
    ...
    def test_01_initialization_in_test_mode(self):
        ...
        self.assertTrue(self.tracker.test_mode, "Tracker should auto-detect the unittest environment.")
    ...
    def test_02_track_error_increments_count(self):
        ...
        self.tracker.track_error('test_component', 'test_type', ValueError("Test error"))
        self.assertEqual(self.tracker.error_counts.get(error_key), 1)
        self.tracker.track_error('test_component', 'test_type', ValueError("Another error"))
    ...
    def test_03_component_becomes_unhealthy_at_threshold(self):
        ...
        self.assertTrue(self.tracker.component_health[component], "Component should be healthy initially.")

        # Log errors up to one BEFORE the threshold
        for i in range(threshold - 1):
            self.tracker.track_error(component, error_type, Exception(f"Error {i + 1}"))
            self.assertTrue(self.tracker.component_health[component], f"Component should be healthy after {i + 1} errors.")

        # Log the final error that MEETS the threshold
        self.tracker.track_error(component, error_type, Exception(f"Error {threshold}"))
        self.assertFalse(self.tracker.component_health[component], "Component should become unhealthy exactly at the threshold.")
    ...
    def test_04_health_does_not_change_in_default_test_mode(self):
        ...
        self.assertTrue(self.tracker.component_health[component], "Component health should not change in test mode.")
