...
def mock_models():
    ...
    with patch('transformers.pipeline') as mock_pipeline, \
         ...
         patch('tensorflow.keras.models.load_model') as mock_tf_load:
        
        # Mock model predictions
        mock_pipeline.return_value = Mock(return_value=[{'label': 'happy', 'score': 0.8}])
...
class TestSystemIntegration:
    ...
    def test_error_propagation(self, emotional_service):
        ...
        with patch.object(emotional_service.emotion_detector, 'detect_text_emotion',
    ...
    def test_resource_limits(self, emotional_service):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            results = list(executor.map(
                emotional_service.generate_response,
                large_batch
            ))
...
class TestEmotionalPerformance:
    ...
    def test_response_generation_performance(self, emotional_service, test_data):
        ...
        logger.info(f"Response generation - Avg: {avg_response_time:.3f}s, P95: {p95_response_time:.3f}s")
    ...
    def test_concurrent_processing(self, emotional_service):
        ...
        def process_emotions():
            ...
            for _ in range(num_requests_per_thread):
                try:
                    result = emotional_service.detect_emotions(
    ...
    def test_batch_processing_efficiency(self, emotional_service):
        ...
        for batch_size in batch_sizes:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                results = list(executor.map(
                    lambda x: emotional_service.detect_emotions(text=x),
                    requests
                ))
            duration = time.time() - start_time
        ...
        assert all(processing_times[size] <= processing_times[1] 
    ...
    def test_resource_utilization(self, emotional_service, test_data):
        ...
        assert max_memory - min(memory_usage) < 200 * 1024 * 1024  # Memory growth under 200MB
        
        logger.info(f"CPU usage - Avg: {avg_cpu:.1f}%, Max: {max_cpu:.1f}%")
        logger.info(f"Memory usage - Avg: {avg_memory/1024/1024:.1f}MB, Max: {max_memory/1024/1024:.1f}MB")