...
from alejo.utils.exceptions import (
    LLMServiceError, CommandError, VoiceRecognitionError
)
...
class TestALEJOBrain(unittest.TestCase):
    ...
    def test_brain_initialization_with_context_managers(self):
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
            with patch.dict(os.environ, {"ALEJO_LOCAL_INFERENCE": "1"}):
                ...
                with patch('alejo.llm_client.local_client.LocalLLMClient', return_value=mock_llm_instance):
    ...
    def test_process_command_executes_successfully(self):
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
                with patch('openai.OpenAI') as mock_OpenAI_class:
                    ...
                    mock_llm_instance.chat.completions.create.return_value.choices[0].message.content = '{"command": "test", "args": "test args"}'
                    
                    # Configure mock command processor response
                    mock_cmd_processor.execute_command.return_value = "Command executed successfully"
                    ...
                    mock_cmd_processor.execute_command.assert_called_once_with("test", "test args")
    ...
    def test_error_handling_llm_failure(self):
        ...
        mock_llm_instance.chat.completions.create.side_effect = LLMServiceError("LLM service failed")
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
            with patch.dict(os.environ, {"ALEJO_LOCAL_INFERENCE": "1"}):
                with patch('alejo.llm_client.local_client.LocalLLMClient', return_value=mock_llm_instance):
                    ...
                    result = brain.process_command("test command")
                    self.assertEqual(result, "I seem to be having trouble connecting to my reasoning core at the moment.")
    ...
    def test_error_handling_command_failure(self):
        ...
        mock_llm_instance.chat.completions.create.return_value.choices[0].message.content = json.dumps({
    ...
    def test_error_handling_voice_failure(self):
        ...
        mock_llm_instance.audio.transcriptions.create.side_effect = Exception("Transcription failed")
            ...
            with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
                with patch.dict(os.environ, {"ALEJO_LOCAL_INFERENCE": "1"}):
                    with patch('alejo.llm_client.local_client.LocalLLMClient', return_value=mock_llm_instance):
                        ...
                        # Verify the error message
                        self.assertIn("Voice recognition failed: Transcription failed", str(cm.exception))
    ...
    def test_successful_voice_processing(self):
        ...
        mock_llm_instance.audio.transcriptions.create.return_value = "turn on the lights"
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
                with patch('openai.OpenAI') as mock_OpenAI_class:
                    with patch('alejo.utils.metrics.get_metrics') as mock_get_metrics:
                        with patch('alejo.utils.monitoring.get_monitor') as mock_get_monitor:
                            # Configure mocks
                            ...
                            self.assertEqual(health_status['recovery_stats']['total_recoveries'], 10)
    ...
    def test_process_command_handles_natural_language_response(self):
        ...
        # Configure LLM response
        mock_llm_instance.chat.completions.create.return_value.choices[0].message.content = 'This is a natural language response'
    ...
    with patch.dict(os.environ, {"ALEJO_LOCAL_INFERENCE": "1"}):
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
            with patch('openai.OpenAI') as mock_OpenAI_class:
                ...
                valid_command_json = '{"command": "delete_file", "args": "protected.txt"}'
                mock_response = MagicMock()
                ...
                mock_cmd_processor.execute_command.assert_called_once_with("delete_file", "protected.txt")
    ...
    def test_process_command_handles_llm_query_failure(self):
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
                with patch('openai.OpenAI') as mock_OpenAI_class:
                    ...
                    mock_llm_instance.chat.completions.create.side_effect = Exception(error_message)
                    ...
                    self.assertEqual(final_response, "I seem to be having trouble connecting to my reasoning core at the moment.")
    ...
    def test_process_command_relays_decorator_error_message(self):
        ...
        with patch.dict('sys.modules', {'alejo.commands': mock_commands}):
                with patch('openai.OpenAI') as mock_OpenAI_class:
                    ...
                    # Mock the LLM's response to be a valid command
                    valid_command_json = '{"command": "create_file", "args": "/unauthorized/path/file.txt"}'
                    mock_response = MagicMock()
                    mock_response.choices[0].message.content = valid_command_json
                    mock_llm_instance.chat.completions.create.return_value = mock_response
                    ...
                    # 3. Process a command
                    final_response = brain.process_command("create a file in a forbidden directory")

                    # 4. Assertions
                    mock_cmd_processor.execute_command.assert_called_once_with("create_file", "/unauthorized/path/file.txt")
