...
def mock_brain(config):
    ...
    async def emit_test_events():
        ...
        await brain.event_bus.emit("brain.emotion_analyzed", {
            "emotions": {
                "joy": 0.8,
                "sadness": 0.1,
                "anger": 0.05,
        ...
        await brain.event_bus.emit("brain.sentiment_analyzed", {
            "sentiment": 0.75
        })
        
        # Emit ethical decision event
        await brain.event_bus.emit("brain.ethical_decision", {
            "decision": {
                "action": "approve",
                "evaluation": "positive",
                "principles": {
        ...
        await brain.event_bus.emit("brain.interaction_recommendation", {
...
async def test_emotional_insights_tab(page, ui_server, mock_brain):
    ...
    recommendation_box = await page.query_selector('input[aria-label="Interaction Recommendations"]')
...
async def test_settings_tab(page, ui_server):
    ...
    # Change settings
    await page.select_option('select[aria-label="Default LLM Model"]', "local/mistral-7b")
