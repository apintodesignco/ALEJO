...
async def test_initialization(config_file):
    ...
    with patch('alejo.emotional_intelligence.ethics.EthicalFramework') as mock_framework_class:
...
async def test_update_ethical_principle(ethical_integration, mock_ethical_framework):
    ...
    with patch.object(mock_ethical_framework, 'update_principle_weight', side_effect=ValueError("Unknown principle")):
        result = await ethical_integration.update_ethical_principle(
...
async def test_get_recent_decisions(ethical_integration, mock_ethical_framework):
    ...
    assert result["decisions"][1]["action"] == "Share personal information about the user"
...
async def test_event_handlers(ethical_integration):
    ...
    result = await ethical_integration._handle_get_recent_decisions({
        "limit": 5
    })
...
async def test_error_handling(ethical_integration):
    ...
    with patch.object(ethical_integration.ethical_framework, 'evaluate_action', side_effect=Exception("Test error")):
