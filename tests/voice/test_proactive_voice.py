...
class TestProactiveVoice(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.voice_service = VoiceService(
            self.brain, self.config, event_bus=self.event_bus
        )
    ...
    async def test_event_subscription(self):
        ...
        voice_service = VoiceService(self.brain, self.config, event_bus=self.event_bus)
        ...
        self.event_bus.subscribe.assert_called_once_with(
            EventType.PROACTIVE_PROMPT, voice_service._handle_proactive_prompt
        )
