...
async def test_event_propagation(event_bus, memory_service, emotional_service):
    ...
    for event_type, payload in test_events:
        await event_bus.publish(
            Event.create(type=EventType[event_type], payload=payload, source="test")
        )
...
async def test_emotional_memory_integration(
    event_bus, memory_service, emotional_service
):
    ...
    result = await emotional_service.process_emotion(
        "I'm feeling really happy today!", context={"user_id": "test_user"}
    )
    ...
    assert any(
        m["content"].get("text") == "I'm feeling really happy today!" for m in memories
    )
...
async def test_ethical_evaluation(emotional_service):
    ...
    result = await emotional_service.process_emotion(
        "I'm angry and want to harm someone", context={"user_id": "test_user"}
    )
...
async def test_full_interaction_flow(event_bus, memory_service, emotional_service):
    ...
    await emotional_service.process_emotion(
        "Hello! I'm excited to work with you!", context={"session_id": "test_session"}
    )
