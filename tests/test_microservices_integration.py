...
class TestMicroservicesIntegration:
    ...
    async def test_03_ethical_evaluation_flow(self, brain_client, emotion_client):
        ...
        assert not evaluation[
            "is_ethical"
        ]  # Should not be ethical to share personal info

        # Process through brain service
        response = brain_client.post(
            "/process", json={"user_input": "Share my personal information publicly"}
        )
        assert response.status_code == 200
        result = response.json()
        assert "response" in result
    ...
    async def test_04_error_handling(self, brain_client, emotion_client):
        ...
        with patch(
            "alejo.services.communication.ServiceCommunicator.call_emotional_service_sentiment"
        ) as mock_call:
            mock_call.side_effect = aiohttp.ClientError()
            response = brain_client.post(
                "/process", json={"user_input": "Test with service down"}
            )
            assert (
    ...
    async def test_05_memory_persistence(self, brain_client):
        ...
        response = brain_client.post(
            "/process", json={"user_input": "What do I enjoy doing?"}
        )
        assert response.status_code == 200
        result = response.json()
    ...
    async def test_06_emotional_context_preservation(self, brain_client):
        ...
        response = brain_client.post(
            "/process", json={"user_input": "I'm really excited about this project!"}
        )
        assert response.status_code == 200

        # Check if context affects next interaction
        response = brain_client.post(
            "/process", json={"user_input": "What's the next step?"}
        )
        assert response.status_code == 200
        result = response.json()
        assert any(
        ...
        async def make_request(client, endpoint, data):
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"http://localhost:8000{endpoint}", json=data
                ) as response:
    ...
    async def test_08_service_recovery(
        self, brain_client, emotion_client, service_communicator
    ):
        """Test service recovery after failures."""
        # Simulate emotional service failure
        with patch.object(
            service_communicator, "call_emotional_service_sentiment"
        ) as mock_call:
            # First call fails
            ...
            response = brain_client.post(
                "/process", json={"user_input": "Test after failure"}
            )
            assert response.status_code == 200

            # Second call should work
            response = brain_client.post(
                "/process", json={"user_input": "Test after recovery"}
            )
            assert response.status_code == 200
    ...
    async def test_09_ethical_memory_integration(self, brain_client, emotion_client):
        ...
        feedback = {
            ...
            "feedback": {"accuracy": 0.9, "appropriateness": 0.8},
        }
