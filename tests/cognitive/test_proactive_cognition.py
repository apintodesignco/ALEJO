...
from alejo.cognitive.proactive_cognition import (
    CognitiveInsight,
    ProactiveCognitionEngine,
)
...
def proactive_engine(mock_config_manager, mock_event_bus):
    ...
    with patch("alejo.cognitive.proactive_cognition.EmpathyEngine"), patch(
        "alejo.cognitive.proactive_cognition.CuriosityEngine"
    ):
...
class TestProactiveCognitionEngine:
    ...
    def test_initialization(
        self, proactive_engine, mock_config_manager, mock_event_bus
    ):
    ...
    def test_register_event_handlers(self, mock_event_bus):
        ...
        with patch("alejo.cognitive.proactive_cognition.EmpathyEngine"), patch(
            ...
            mock_event_bus.subscribe.assert_any_call(
                "message.received", engine._handle_message_received
            )
            mock_event_bus.subscribe.assert_any_call(
                "user.emotion_detected", engine._handle_emotion_detected
            )
            mock_event_bus.subscribe.assert_any_call(
                "conversation.topic_changed", engine._handle_topic_changed
            )
    ...
    async def test_handle_message_received(self, proactive_engine):
        ...
        proactive_engine.empathy_engine.record_turn.assert_called_once_with(
            "user", "Hello, world!"
        )
        proactive_engine.curiosity_engine.record_turn.assert_called_once_with(
            "user", "Hello, world!"
        )

        # Verify message processing was called
        proactive_engine._process_message.assert_called_once_with(
            "user", "Hello, world!"
        )
    ...
    async def test_handle_emotion_detected(self, proactive_engine):
        ...
        initial_receptiveness = proactive_engine.conversation_state[
            "user_receptiveness"
        ]
        ...
        assert (
            proactive_engine.conversation_state["emotional_trajectory"][0]["emotion"]
            == "angry"
        )

        # Verify receptiveness decreased for negative emotion
        assert (
            proactive_engine.conversation_state["user_receptiveness"]
            < initial_receptiveness
        )
        ...
        current_receptiveness = proactive_engine.conversation_state[
            "user_receptiveness"
        ]
    ...
    async def test_handle_topic_changed(self, proactive_engine):
        ...
        assert (
            proactive_engine.conversation_state["topic_focus"]
    ...
    async def test_process_message_empathy(self, proactive_engine):
        ...
        # Configure mocks
        proactive_engine.empathy_engine = MagicMock()
        proactive_engine.empathy_engine.should_reflect.return_value = MagicMock(
            confidence=0.8
        )
        proactive_engine.empathy_engine.generate_reflection.return_value = (
        ...
        await proactive_engine._process_message(
            "user", "I'm feeling frustrated with this."
        )
    ...
    def test_adjust_curiosity_threshold(self, proactive_engine):
        ...
        assert (
            threshold > proactive_engine.curiosity_threshold
