...
from alejo.cognitive.memory.semantic_memory import Concept, SemanticMemory
...
from alejo.database.memory_store import Memory, MemoryStore
...
def semantic_memory():
    ...
    memory.concepts = {
        ...
        "RAG": Concept(
            ...
            relationships=[{"type": "used_by", "target": "ALEJO"}],
            confidence=0.8,
            source="learning",
        ),
    }
...
@pytest.fixture
def retrieval_integration(
    event_bus, memory_store, semantic_memory, config_path, patch_llm_factory
):
...
async def test_integration_mode_setting(retrieval_integration):
    ...
    result = await retrieval_integration._handle_set_mode({"mode": "rag"})
    ...
    result = await retrieval_integration._handle_set_mode({"mode": "invalid"})
