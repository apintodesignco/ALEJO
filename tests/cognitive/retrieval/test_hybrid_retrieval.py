...
from alejo.cognitive.retrieval.cag_system import CAGConfig, CAGSystem
from alejo.cognitive.retrieval.cloud_cache import (
    CloudCache,
    CloudCacheConfig,
    CloudStorageProvider,
)
from alejo.cognitive.retrieval.hybrid_retrieval import (
    HybridConfig,
    HybridRetrievalSystem,
)
from alejo.cognitive.retrieval.rag_system import RAGConfig, RAGSystem
...
def hybrid_system(event_bus, mock_llm_client, rag_system, cag_system, cloud_cache):
    ...
    config = HybridConfig(
        default_mode="auto", hybrid_weight_rag=0.6, use_cloud_cache=True
    )
...
async def test_hybrid_mode_selection(hybrid_system):
    ...
    mode = await hybrid_system.determine_best_mode(
        "You said something earlier about ALEJO"
    )
...
async def test_hybrid_response_generation(hybrid_system):
    ...
    hybrid_system.cag_system.add_context(
        "User asked about ALEJO's capabilities", "user", "test_cag"
    )
...
async def test_cloud_cache_integration(hybrid_system, temp_cache_dir):
    ...
    # Test upload
    success = hybrid_system.cloud_cache.upload(
        test_file_path, "test_remote.txt", sync=True
    )
    assert success is True
    ...
    success = hybrid_system.cloud_cache.download(
        "test_remote.txt", download_path, sync=True
    )
    assert success is True
