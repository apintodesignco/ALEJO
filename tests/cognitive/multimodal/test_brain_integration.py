...
async def test_initialization():
    ...
    with patch(
        "alejo.cognitive.multimodal.processor.MultimodalProcessor"
...
async def test_process_image_for_brain_question(multimodal_integration, temp_image):
    ...
    result = await multimodal_integration.process_image_for_brain(
        image_path=temp_image, query="What is in this image?", user_id="test_user"
    )
...
async def test_process_image_for_brain_general(multimodal_integration, temp_image):
    ...
    result = await multimodal_integration.process_image_for_brain(
        image_path=temp_image, query="This is an interesting image", user_id="test_user"
    )
...
async def test_caption_style_detection(multimodal_integration):
    ...
    assert (
        multimodal_integration._determine_caption_style("Give me a brief description")
        == "concise"
    )
    assert (
        multimodal_integration._determine_caption_style("Describe this in detail")
        == "detailed"
    )
    assert (
        multimodal_integration._determine_caption_style("Create a creative caption")
        == "creative"
    )
    assert (
        multimodal_integration._determine_caption_style("What's in this image?")
        == "descriptive"
    )
...
async def test_description_request_detection(multimodal_integration):
    ...
    assert not multimodal_integration._is_description_request(
...
async def test_event_handlers(multimodal_integration, temp_image):
    ...
    result = await multimodal_integration._handle_brain_visual_qa(
        {"image_path": temp_image, "question": "How many people?"}
    )
    ...
    result = await multimodal_integration._handle_brain_analyze_scene(
        {"image_path": temp_image, "analysis_type": "detailed"}
    )
    ...
    result = await multimodal_integration._handle_brain_caption_image(
        {"image_path": temp_image, "style": "creative"}
    )
...
async def test_error_handling(multimodal_integration, temp_image):
    ...
    with patch.object(
        multimodal_integration.multimodal_processor,
        "visual_qa",
