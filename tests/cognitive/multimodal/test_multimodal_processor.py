...
from alejo.vision.scene_analyzer import EnvironmentalContext, SceneContext, SceneObject
    ...
    async def analyze_scene(self, image_path):
        ...
        return SceneContext(
            ...
            environment=EnvironmentalContext(
                lighting="bright", setting="outdoor", time_of_day="day"
            ),
            scene_type="garden",
            confidence=0.9,
        )
...
def multimodal_processor(
    temp_dir, mock_vlm_client, mock_vision_processor, mock_memory_store, mock_event_bus
):
    ...
    with patch(
        "alejo.llm_client.factory.LLMClientFactory.create_client",
...
async def test_process_image_with_text(multimodal_processor, temp_image):
    ...
    result = await multimodal_processor.process_image_with_text(
        image_path=temp_image, prompt="Describe this image"
    )
...
async def test_analyze_scene(multimodal_processor, temp_image):
    ...
    result = await multimodal_processor.analyze_scene(
        image_path=temp_image, analysis_type="comprehensive"
    )
...
async def test_visual_qa(multimodal_processor, temp_image):
    ...
    result = await multimodal_processor.visual_qa(
        image_path=temp_image, question="How many flowers are in the image?"
    )
...
async def test_caption_image(multimodal_processor, temp_image):
    ...
    result = await multimodal_processor.caption_image(
        image_path=temp_image, style="descriptive"
    )
...
async def test_event_handlers(multimodal_processor, temp_image, mock_event_bus):
    ...
    result = await handler({"image_path": temp_image, "prompt": "Describe this image"})
...
async def test_error_handling(multimodal_processor, temp_image):
    ...
    with patch.object(
        multimodal_processor, "vlm_client", side_effect=Exception("Test error")
    ):
