...
async def stream_manager(vision_processor):
    ...
    config = StreamManagerConfig(
        enable_gpu=True, max_batch_size=4, buffer_size=30, frame_rate=30
    )
    manager = StreamManager(
        vision_processor=vision_processor, event_bus=EventBus(), config=config
    )
    yield manager
    await manager.stop()
...
class TestVisionBenchmarks:
    ...
    async def test_single_frame_processing(
        self, vision_processor: VisionProcessor, sample_frames: List[np.ndarray]
    ):
        """Benchmark single frame processing"""
        frame = sample_frames[0]

        @benchmarker.benchmark(
            name="vision_processor", operation="single_frame", iterations=100
        )
    ...
    async def test_stream_processing(
        self, stream_manager: StreamManager, sample_frames: List[np.ndarray]
    ):
        """Benchmark stream processing"""

        @benchmarker.benchmark(
            name="stream_manager", operation="process_stream", iterations=10
        )
    ...
    async def test_compare_gpu_cpu(
        ...
        @benchmarker.benchmark(
            name="vision_processor", operation="cpu_processing", iterations=50
        )
        ...
        if torch.cuda.is_available():
            ...
            @benchmarker.benchmark(
                name="vision_processor", operation="gpu_processing", iterations=50
            )
            ...
            assert (
                gpu_result.mean_time < cpu_result.mean_time * 0.5
