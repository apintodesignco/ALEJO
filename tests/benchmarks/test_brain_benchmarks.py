...
class TestBrainBenchmarks:
    ...
    async def test_text_processing(self, brain_service):
        ...
        @benchmarker.benchmark(
            name="brain_service", operation="process_text", iterations=50
        )
        ...
        assert (
            texts_per_second >= 10
        ), f"Text processing too slow: {texts_per_second:.1f} texts/s"
    ...
    async def test_multimodal_fusion(self, multimodal_processor):
        ...
        @benchmarker.benchmark(
            name="multimodal_processor", operation="fuse_modalities", iterations=100
        )
    ...
    async def test_proactive_dialogue(self, alejo_brain):
        ...
        @benchmarker.benchmark(
            name="alejo_brain", operation="check_proactive", iterations=200
        )
    ...
    async def test_command_processing(self, brain_service):
        ...
        @benchmarker.benchmark(
            name="brain_service", operation="process_command", iterations=50
        )
        ...
        assert (
            cmds_per_second >= 20
        ), f"Command processing too slow: {cmds_per_second:.1f} cmds/s"
    ...
    async def test_concurrent_brain_operations(self, brain_service, alejo_brain):
        ...
        @benchmarker.benchmark(
            name="brain_system", operation="concurrent_ops", iterations=20
        )
        ...
        assert (
            ops_per_second >= 50
        ), f"Concurrent operations too slow: {ops_per_second:.1f} ops/s"
    ...
    async def test_memory_impact(self, brain_service):
        ...
        assert (
            result.memory_usage < 500
        ), f"Memory usage too high: {result.memory_usage:.1f} MB"
