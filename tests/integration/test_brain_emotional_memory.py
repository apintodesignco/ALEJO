...
class TestBrainEmotionalMemoryIntegration:
    ...
    def mock_llm_client(self):
        ...
        mock_client.generate_text = AsyncMock(return_value=LLMResponse(
            text="This is a test response",
            metadata={"model": "mock-model", "tokens": 10}
        ))
        mock_client.generate_chat_response = AsyncMock(return_value=LLMResponse(
    ...
    def brain(self, event_bus, config, mock_llm_client):
        ...
        with patch('alejo.brain.alejo_brain.LLMClientFactory.create_client', 
    ...
    async def test_emotional_state_update_from_llm_response(self, brain, event_bus):
        ...
        with patch.object(brain.emotional_processor, 'analyze_text', return_value={
            "valence": 0.8,
            "arousal": 0.6,
            "dominance": 0.7,
            ...
            await event_bus.emit("llm.response", {
    ...
    @pytest.mark.asyncio
    async def test_store_interaction_during_text_processing(self, brain, mock_llm_client):
        ...
        with patch.object(brain.emotional_processor, 'analyze_text', return_value={
            ...
            "confidence": 0.85
        }), patch.object(brain.emotional_memory, 'store_interaction', AsyncMock()) as mock_store:
    ...
    async def test_relationship_update_after_interaction(self, brain):
        ...
        with patch.object(brain.emotional_processor, 'analyze_text', return_value={
            ...
            "confidence": 0.95
        }), patch.object(brain.emotional_memory, 'update_relationship', AsyncMock()) as mock_update:
    ...
    async def test_value_system_integration(self, brain):
        ...
        # Mock store_value
        with patch.object(brain.emotional_memory, 'store_value', AsyncMock()) as mock_store:
            # Emit a value update event
            await brain.event_bus.emit("value.update", {
                "value_name": "honesty",
                "value_level": 0.95
            })
    ...
    async def test_emotional_patterns_in_response_generation(self, brain):
        ...
        with patch.object(brain.emotional_memory, 'get_emotional_patterns', AsyncMock(return_value=[
            {
                "type": "trend",
                "data": {"dimension": "valence", "trend": "increasing"},
                "confidence": 0.8
            }
        ])), patch.object(brain.emotional_processor, 'adjust_response', return_value="Adjusted response") as mock_adjust:
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_emotional_memory_integration(self, brain, event_bus):
        """Test error handling in emotional memory integration."""
        # Initialize the brain
        await brain.emotional_memory.initialize()
        
        # Mock store_interaction to raise an error
        with patch.object(brain.emotional_memory, 'store_interaction', 
    ...
    async def test_brain_shutdown_with_emotional_memory(self, brain):
        ...
        with patch.object(brain.emotional_memory, 'update_relationship', AsyncMock()) as mock_update:
            # Shutdown the brain
