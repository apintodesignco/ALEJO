...
class TestBrainMultimodalIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("alejo.brain.alejo_brain.LLMClientFactory"), patch(
            "alejo.brain.alejo_brain.EmotionalMemory"
        ), patch("alejo.brain.alejo_brain.EmotionalProcessor"), patch(
            "alejo.brain.alejo_brain.EthicalFramework"
        ), patch(
            "alejo.brain.alejo_brain.AdaptiveEmotionalProcessor"
        ), patch(
            "alejo.brain.alejo_brain.get_command_processor"
        ):
    ...
    async def test_process_multimodal_input(
        self, mock_setup, mock_get_emotional, mock_integration_class
    ):
        ...
        self.brain.emotional_processor.enhance_response = AsyncMock(
            return_value="Enhanced response"
        )
        ...
        mock_integration.process_text_image = AsyncMock(
            return_value={
                "success": True,
                "result": {
        ...
        result = await self.brain.process_multimodal_input(
            "What is in this image?", "test_image_data"
        )
    ...
    async def test_optimize_component(self, mock_setup, mock_integration_class):
        ...
        mock_integration.optimize_component = AsyncMock(
            return_value={
                "success": True,
        ...
        result = await self.brain.optimize_component(
            "test_component", "def test(): pass"
        )
    ...
    async def test_hot_swap_model(self, mock_setup, mock_integration_class):
        ...
        mock_integration.hot_swap_model = AsyncMock(
            return_value={
                "success": True,
                "model_id": "test_model",
                "task_type": "vqa",
    ...
    async def test_process_multimodal_result(self, mock_integration_class):
        ...
        self.brain.event_bus.emit.assert_called_once_with(
            "brain.processing_result",
            {
                "type": "multimodal",
                "query_id": "test_query",
                "success": True,
