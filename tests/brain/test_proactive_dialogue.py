...
async def brain():
    ...
    config = {
        'proactive_dialogue': {
            'min_interval_seconds': 5  # Short interval for testing
        }
    }
    event_bus = EventBus()
...
async def test_proactive_dialogue_timing(brain):
    ...
    brain.adaptive_processor.get_proactive_question.return_value = "How are you feeling?"
...
async def test_graceful_failure_emotional_components(brain):
    ...
    brain.adaptive_processor.get_proactive_question.side_effect = Exception("Service unavailable")
...
async def test_conversation_state_management(brain):
    ...
    responses = []
    async for response in brain.process_text("I'm feeling down today", is_proactive_response=True):
        responses.append(response)
    ...
    brain.adaptive_processor.get_proactive_question.return_value = "Would you like to talk about what's bothering you?"
    brain._last_proactive_check = datetime.now() - timedelta(seconds=10)
