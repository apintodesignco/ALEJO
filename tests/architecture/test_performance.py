...
class PerformanceMonitor:
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            events_per_second=(
                self.event_count / elapsed_time if elapsed_time > 0 else 0
            ),
...
async def test_event_processing_performance(
    ...
    for i in range(EVENT_COUNT):
        await event_bus.publish(
            Event(type=EventType.BRAIN_THOUGHT, data={"thought": f"Test thought {i}"})
        )
    ...
    ), f"Average latency too high: {metrics.avg_latency}ms"
    assert (
        metrics.events_per_second > 100
    ), f"Event throughput too low: {metrics.events_per_second}/s"
...
async def test_memory_system_performance(
    working_memory: WorkingMemory, performance_monitor: PerformanceMonitor
):
    ...
    for i in range(MEMORY_COUNT):
        ...
        await working_memory.store_memory(
            {"type": "test", "content": f"Test memory {i}", "context": {"test": True}}
        )
    ...
    assert (
        metrics.service_latencies["memory_retrieval"] < 10
    ...
    logger.info(
        f"Retrieval Latency: {metrics.service_latencies['memory_retrieval']:.2f}ms"
    )


async def test_service_mesh_performance(
    service_mesh: ServiceMesh, performance_monitor: PerformanceMonitor
):
...
class TestComponentPerformance:
    ...
    async def test_command_throughput(self, setup_test):
        ...
        performance_monitor.record_latency(
            "command_processing", duration / num_commands
        )

        # Should handle at least 10 commands per second
        assert (
            throughput > 10.0
        ), f"Command throughput {throughput:.2f} commands/sec below threshold"
    ...
    async def test_emotional_processor_performance(self, setup_test):
        ...
        # Record metrics
        performance_monitor.record_latency(
            "emotional_processing", duration / num_iterations
        )

        # Should handle at least 50 sentiment analyses per second
        assert (
            throughput > 50.0
        ), f"Emotional processing throughput {throughput:.2f} ops/sec below threshold"
...
async def test_service_mesh_performance(
    ...
    for i in range(REQUEST_COUNT):
        ...
        response = await service_mesh.send_request(
            "test_service", "handle_request", {"test": True}
        )
        latency = (time.time() - start_time) * 1000
    ...
    logger.info(
        f"Request Latency: {metrics.service_latencies['service_request']:.2f}ms"
    )
