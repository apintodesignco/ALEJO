...
class MockMultimodalIntegration:
    ...
    async def process_image_for_brain(
        self, image_path, query, user_id=None, context=None
    ):
        """Mock process image"""
        return {
...
class MockEmotionalIntelligenceIntegration:
    ...
    async def get_emotional_response(
        self, input_text, user_id=None, context=None, target_emotion=None
    ):
    ...
    async def get_ethical_principles(self):
        ...
        return {
            "principles": {
                ...
                "privacy": {"description": "Protect user privacy", "weight": 0.95},
            }
        }
...
class MockRetrievalIntegration:
    ...
    async def learn(self, content, metadata=None, user_id=None):
        ...
        return {"success": True, "chunk_count": 3, "id": "doc_123"}
...
def config_file(temp_dir):
    ...
    config = {
        ...
        "extension_configs": {
            "multimodal": {"default_reasoning_depth": "deep"},
            "emotional_intelligence": {"empathy_threshold": 0.8},
        },
    }
...
def brain_extensions(mock_brain, monkeypatch):
    ...
    monkeypatch.setattr(
        "alejo.cognitive.multimodal.integration.MultimodalIntegration",
        lambda **kwargs: MockMultimodalIntegration(),
    )
    monkeypatch.setattr(
        "alejo.emotional_intelligence.integration.EmotionalIntelligenceIntegration",
        lambda **kwargs: MockEmotionalIntelligenceIntegration(),
    )
    monkeypatch.setattr(
        "alejo.ethical.integration.EthicalIntegration",
        lambda **kwargs: MockEthicalIntegration(),
    )
    monkeypatch.setattr(
        "alejo.cognitive.retrieval.integration.RetrievalIntegration",
...
async def test_initialization(config_file):
    ...
    with patch("alejo.core.brain.ALEJOBrain") as mock_brain_class:
        ...
        with patch(
            "alejo.cognitive.multimodal.integration.MultimodalIntegration"
        ) as mock_multimodal_class, patch(
            "alejo.emotional_intelligence.integration.EmotionalIntelligenceIntegration"
        ) as mock_emotional_class, patch(
            "alejo.ethical.integration.EthicalIntegration"
        ) as mock_ethical_class, patch(
            "alejo.cognitive.retrieval.integration.RetrievalIntegration"
            ...
            assert (
                extensions.config["extension_configs"]["emotional_intelligence"][
                    "empathy_threshold"
                ]
                == 0.8
            )
...
async def test_multimodal_methods(brain_extensions):
    ...
    result = await brain_extensions._process_image(
        image_path="/path/to/image.jpg", query="What's in this image?"
    )
    ...
    result = await brain_extensions._visual_qa(
        image_path="/path/to/image.jpg", question="What color is the car?"
    )
    ...
    result = await brain_extensions._analyze_scene(
        image_path="/path/to/image.jpg", analysis_type="detailed"
    )
    ...
    result = await brain_extensions._caption_image(
        image_path="/path/to/image.jpg", style="poetic"
    )
...
async def test_emotional_intelligence_methods(brain_extensions):
    ...
    result = await brain_extensions._analyze_emotion(text="I'm feeling happy today!")
    ...
    result = await brain_extensions._get_emotional_response(
        input_text="I just got a promotion!", user_id="user123"
    )
    ...
    result = await brain_extensions._get_interaction_recommendation(
        user_id="user123", current_context={"recent_sentiment": "positive"}
    )
...
async def test_hybrid_retrieval_methods(brain_extensions):
    ...
    result = await brain_extensions._retrieve_context(
        query="How does photosynthesis work?", mode="hybrid"
    )
    ...
    result = await brain_extensions._set_retrieval_mode(
        mode="rag_only", rag_weight=0.8, cag_weight=0.2
    )
...
async def test_disabled_extensions(brain_extensions):
    ...
    with pytest.raises(BrainExtensionError):
        await brain_extensions._analyze_emotion(text="I'm feeling happy today!")
...
async def test_error_handling(brain_extensions):
    ...
    with patch.object(
        brain_extensions.multimodal,
        "process_image_for_brain",
