...
async def test_io_monitoring(io_optimizer):
    # Mock disk I/O data
    mock_counters = Mock(
        read_bytes=1024 * 1024, write_bytes=2 * 1024 * 1024  # 1MB  # 2MB
    )
...
async def test_hot_path_detection(io_optimizer, temp_dir):
    ...
    mock_process.io_counters.return_value = Mock(
        read_bytes=5 * 1024 * 1024, write_bytes=10 * 1024 * 1024  # 5MB  # 10MB
    )
...
async def test_optimization_suggestions(io_optimizer):
    ...
    mock_counters = Mock(
        read_bytes=50 * 1024 * 1024, write_bytes=100 * 1024 * 1024  # 50MB  # 100MB
    )
    ...
    with patch("psutil.disk_io_counters", return_value={"sda": mock_counters}):
        ...
        assert any(
            s["type"] in ("high_read_rate", "high_write_rate") for s in suggestions
        )
...
async def test_monitoring_lifecycle(io_optimizer):
    ...
    monitor_task = asyncio.create_task(io_optimizer.start_monitoring())
...
async def test_log_rotation(io_optimizer, temp_dir):
    ...
    mock_process.io_counters.return_value = Mock(
        read_bytes=0, write_bytes=1024 * 1024 * 200  # 200MB written
    )
    ...
    with patch("psutil.process_iter", return_value=[mock_process]):
        ...
        assert any(
            s["type"] == "hot_path" and "test.log" in s["path"] for s in suggestions
        )
...
async def test_error_handling(io_optimizer):
    ...
    with patch("psutil.disk_io_counters", side_effect=psutil.Error("Disk error")):
