...
async def test_service_dependency_events(service_mesh, event_bus):
    ...
    for service, endpoints in services.items():
        for endpoint in endpoints:
            ...
            mock_response.json = AsyncMock(return_value={"dependencies": dependencies})
    ...
    assert (
        "db" not in service_mesh._service_dependencies
        or not service_mesh._service_dependencies["db"]
    )
