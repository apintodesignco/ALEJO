...
import pytest
from alejo.core.event_bus import Event, EventBus, EventBusError, EventType
...
async def test_message_listener():
    ...
    assert mock_pubsub.subscribe.call_count == len(
        EventType
    ), "Should subscribe to all event types"
    assert (
        bus._handle_message.called
    ), "Should call handle_message with the received message"
