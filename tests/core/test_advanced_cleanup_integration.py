...
async def test_memory_leak_handling(cleanup_system):
    ...
    mock_process = Mock()
    mock_process.info = {"memory_info": Mock(rss=1024 * 1024 * 100)}  # Start at 100MB
    mock_process.pid = 12345
    mock_process.name.return_value = "test_process"

    with patch("psutil.process_iter", return_value=[mock_process]), patch(
...
async def test_io_optimization(cleanup_system, temp_dir):
    ...
    with patch("psutil.disk_io_counters", return_value={"sda": mock_counters}):
        ...
        assert any("test.log" in str(sugg.get("path", "")) for sugg in io_suggestions)
