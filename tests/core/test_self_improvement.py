...
from alejo.core.self_improvement import (
...
async def test_event_handlers(self_improvement_engine, mock_brain):
    ...
    await mock_brain.event_bus.emit("brain.message_processed", {"processing_time": 0.5})
    ...
    await mock_brain.event_bus.emit(
        "brain.inference_completed", {"inference_time": 0.3}
    )
    ...
    await mock_brain.event_bus.emit("brain.memory_optimized", {"memory_usage": 512})
    ...
    bottleneck = {"component": "test_component", "issue": "memory leak", "severity": 8}
    await mock_brain.event_bus.emit(
        "brain.bottleneck_detected", {"bottleneck": bottleneck}
    )
...
async def test_suggest_improvements(self_improvement_engine):
    ...
    # Add some test metrics
    self_improvement_engine.metrics["response_times"] = [
        1.5,
        1.6,
        1.7,
        1.8,
    ...
    self_improvement_engine.metrics["memory_usage"] = [
        1500,
        1520,
        1530,
        1525,
...
async def test_component_name_to_test_path(self_improvement_engine):
    ...
    test_path = self_improvement_engine._component_name_to_test_path(
        "alejo.core.brain.Brain"
    )
