...
async def test_memory_leak_detection(memory_detector):
    # Mock process data
    mock_process = Mock()
    mock_process.info = {"memory_info": Mock(rss=1024 * 1024 * 100)}  # 100MB
...
async def test_process_memory_trend(memory_detector):
    # Mock process data
    mock_process = Mock()
    mock_process.info = {"memory_info": Mock(rss=1024 * 1024 * 100)}
...
async def test_monitoring_lifecycle(memory_detector):
    ...
    monitor_task = asyncio.create_task(memory_detector.start_monitoring())
...
async def test_memory_leak_detection_no_leak(memory_detector):
    ...
    mock_process = Mock()
    mock_process.info = {"memory_info": Mock(rss=1024 * 1024 * 100)}  # Constant 100MB
...
async def test_error_handling(memory_detector):
    ...
    mock_process = Mock()
    mock_process.info = {"memory_info": Mock(side_effect=psutil.NoSuchProcess(1234))}
