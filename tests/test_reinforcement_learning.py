...
from alejo.cognitive.learning.reinforcement import Experience, ReinforcementLearner
...
async def test_experience_addition(learner):
    ...
    await learner.add_experience(
        state=state, action="process_normally", reward=1.0, next_state=next_state
    )
...
async def test_learning_manager_event_handling(learning_manager):
    ...
    command_event = Event(
        ...
        metadata={"command_id": "test_1", "emotional_state": {"valence": 0.5}},
    )
    ...
    response_event = Event(
        ...
        payload={"success": True, "satisfaction": 0.8},
