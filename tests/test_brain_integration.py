...
from alejo.commands import CommandProcessor, get_command_processor
...
class TestALEJOBrainIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.command_processor = CommandProcessor(
            config={"error_tracker": self.error_tracker, "test_mode": True}
        )
    ...
    def test_01_command_error_propagation(self):
        ...
        mock_completion.choices[0].message.content = json.dumps(
            {"command": "nonexistent_command", "args": {}}
        )
        ...
        error_count = self.error_tracker.get_error_count(
            "command_processor", "command_execution"
        )
        self.assertGreater(
            error_count, 0, "No errors were tracked for command execution"
        )
        ...
        self.assertIn(
            "Unknown command", str(errors[0]), "Expected error message not found"
        )
    ...
    def test_02_llm_error_recovery(self):
        ...
        responses = [APIError("API Error"), APIError("Rate limit"), mock_completion]
