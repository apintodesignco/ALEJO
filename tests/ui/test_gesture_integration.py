...
class TestGestureIntegration:
    ...
    def test_initialization(self, gesture_integration, event_bus, config_manager):
        ...
        event_bus.subscribe.assert_any_call(
            "gesture.action", gesture_integration.handle_gesture_action
        )
        event_bus.subscribe.assert_any_call(
            "ui.context_changed", gesture_integration.update_ui_context
        )
        event_bus.subscribe.assert_any_call(
            "config.updated.gesture_integration", gesture_integration.update_config
        )
    ...
    async def test_handle_gesture_action_navigation(self, gesture_integration):
        ...
        gesture_integration.handle_navigation_action.assert_awaited_once_with(
            action_event
        )
    ...
    async def test_handle_gesture_action_interaction(self, gesture_integration):
        ...
        await gesture_integration.handle_gesture_action(action_event)
        gesture_integration.handle_interaction_action.assert_awaited_once_with(
            action_event
        )
    ...
    async def test_handle_gesture_action_system(self, gesture_integration):
        ...
        action_event = {"action_type": "system", "command": "undo", "confidence": 0.95}

        await gesture_integration.handle_gesture_action(action_event)
        gesture_integration.handle_system_action.assert_awaited_once_with(action_event)

    @pytest.mark.asyncio
    async def test_handle_gesture_action_low_confidence(
        self, gesture_integration, event_bus
    ):
    ...
    async def test_handle_gesture_action_disabled_handler(self, gesture_integration):
        ...
        assert (
            not hasattr(gesture_integration, "handle_navigation_action")
            or not gesture_integration.handle_navigation_action.called
        )
    ...
    async def test_handle_interaction_action(self, gesture_integration, event_bus):
        ...
        event_bus.publish.assert_called_with(
            "ui.interaction",
            {"type": "select", "element_id": "services-list", "source": "gesture"},
        )
    ...
    async def test_handle_system_action(self, gesture_integration, event_bus):
        ...
        event_bus.publish.assert_called_with(
            "system.command", {"command": "refresh", "source": "gesture"}
        )

    def test_update_ui_context(self, gesture_integration):
        """Test UI context updates."""
        context_event = {"context": "editing", "element_id": "content-editor"}
    ...
    async def test_register_gesture_elements(self, gesture_integration, event_bus):
        ...
        event_bus.publish.assert_called_with(
            "gesture.elements.registered", {"elements": elements, "context": "default"}
        )
    ...
    async def test_announce_gesture_feedback(self, gesture_integration, event_bus):
        ...
        await gesture_integration.announce_gesture_feedback(
            "Zoom gesture recognized", "info"
        )
        event_bus.publish.assert_called_with(
