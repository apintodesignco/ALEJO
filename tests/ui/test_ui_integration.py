...
def emotional_component(mock_brain):
    ...
    return EmotionalInsightsComponent(
        brain=mock_brain, event_bus=mock_brain.event_bus, config={}
    )
...
def alejo_ui(mock_brain):
    ...
    return AlejoUI(brain=mock_brain, config={"ui": {"test": True}})
...
async def test_emotional_component_event_handlers(emotional_component):
    ...
    await emotional_component.handle_sentiment_analysis({"sentiment": 0.75})
...
async def test_multimodal_component_visual_qa(multimodal_component, mock_brain):
    ...
    # Test visual QA
    result = await multimodal_component.handle_visual_qa(
        mock_image, "What's in this image?"
    )

    # Check that brain.visual_qa was called
    mock_brain.visual_qa.assert_called_once_with(
        multimodal_component.current_image_path, "What's in this image?"
    )
