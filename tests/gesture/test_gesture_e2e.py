...
def camera_manager(mock_camera):
    ...
    with patch(
        "alejo.interaction.gesture_arpeggiator.camera_manager.cv2.VideoCapture",
...
async def running_gesture_system():
    ...
    with patch(
        ...
        yield {"arpeggiator": arpeggiator, "server": server}
...
async def test_client_connection_and_preset_change():
    ...
    with patch(
        "websockets.connect",
        new_callable=lambda: asyncio.coroutine(
        ...
        # Create a client
        client = await MockWebSocketClient(
            f"ws://{TEST_SERVER_CONFIG['host']}:{TEST_SERVER_CONFIG['port']}"
        ).connect()

        # Send a preset change message
        preset_message = {"type": "preset_change", "preset": "E2E Test Preset"}
...
async def test_complete_gesture_pipeline(mock_camera):
    ...
    with patch(
        ...
        arpeggiator.gesture_processor.process_frame = MagicMock(
