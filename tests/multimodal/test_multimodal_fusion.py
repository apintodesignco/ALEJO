...
from alejo.multimodal.multimodal_processor import (
    MultimodalProcessor, ProcessingResult, ModalityType
)
...
class TestMultimodalProcessor(unittest.TestCase):
    ...
    def test_analyze_image_with_clip(self, mock_prepare_image):
        ...
        self.processor.models["clip"]["model"].__call__.return_value.logits_per_image = MagicMock()
        
        # Mock torch functions
        with patch('alejo.multimodal.multimodal_processor.torch.argmax') as mock_argmax, \
             patch('alejo.multimodal.multimodal_processor.torch.nn.functional.softmax') as mock_softmax:
    ...
    def test_optimize_with_darwin_godel(self, mock_dgm_class):
        ...
        result = asyncio.run(processor.optimize_with_darwin_godel("test_component", "def test(): pass"))
...
class TestMultimodalIntegration(unittest.TestCase):
    ...
    def test_task_classification(self):
        ...
        self.assertEqual(self.integration._classify_task("What is in this image?"), "vqa")
        self.assertEqual(self.integration._classify_task("Who is the person in the photo?"), "vqa")
        
        # Test caption classification
        self.assertEqual(self.integration._classify_task("Describe this image"), "caption")
        self.assertEqual(self.integration._classify_task("Tell me about this picture"), "caption")
        
        # Test object detection classification
        self.assertEqual(self.integration._classify_task("Find all objects in this image"), "objects")
        self.assertEqual(self.integration._classify_task("Identify people in this photo"), "objects")
        
        # Test scene analysis classification
        self.assertEqual(self.integration._classify_task("What is the scene in this image?"), "scene")
        self.assertEqual(self.integration._classify_task("Describe the environment shown"), "scene")
        
        # Test similarity analysis classification
        self.assertEqual(self.integration._classify_task("How similar is this to the text?"), "similarity")
        self.assertEqual(self.integration._classify_task("Compare this image with the description"), "similarity")
        
        # Test default classification
        self.assertEqual(self.integration._classify_task("Process this image"), "general")
