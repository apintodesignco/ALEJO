...
def config_manager():
    ...
    config = {
        "collective": {"improvements": {"auto_apply": False, "min_confidence": 0.8}},
        "github": {
            "enabled": True,
            "repo": "test/alejo",
            "token": "test_token",
    ...
    manager.get_config.side_effect = lambda key, default=None: (
        config.get(key.split(".")[0], {}).get(key.split(".")[1], default)
        if "." in key
...
class TestImprovementEngine:
    ...
    @pytest.mark.asyncio
    async def test_generate_performance_improvements(
        self, improvement_engine, data_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_security_improvements(
        self, improvement_engine, data_manager
    ):
        ...
        assert (
            improvements[0]["confidence"] >= 0.7
        )  # Security starts with higher base confidence

    @pytest.mark.asyncio
    async def test_generate_accessibility_improvements(
        self, improvement_engine, data_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_privacy_improvements(
        self, improvement_engine, data_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_multiple_category_improvements(
        self, improvement_engine, data_manager
    ):
    ...
    async def test_apply_improvement(self, improvement_engine, data_manager):
        ...
        with patch.object(
            improvement_engine, "_github_api_request", new_callable=AsyncMock
        ) as mock_github:
            mock_github.return_value = {
                "number": 123,
                "html_url": "https://github.com/test/alejo/pull/123",
            }
