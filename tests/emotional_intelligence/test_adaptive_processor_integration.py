...
async def memory_service():
    ...
    service.get_emotional_summary = AsyncMock(
        return_value={
            "interactions": [
                {
    ...
    service.get_emotional_patterns = AsyncMock(
        return_value=[
            {
                "trigger": "achievement",
                "emotion": "joy",
    ...
    service.get_nostalgic_memories = AsyncMock(
        return_value=[
            {
                "timestamp": datetime.now().isoformat(),
                "emotion": "joy",
    ...
    service.get_similar_memories = AsyncMock(
        return_value=[
            {
                "timestamp": datetime.now().isoformat(),
                "emotion": "joy",
...
def event_bus():
    ...
    bus.get_recent_events = MagicMock(
        return_value=[
            Event(
...
def learning_orchestrator():
    ...
    orchestrator.get_interaction_preferences = MagicMock(
        return_value={"pace": "fast", "detail": "brief", "support": "high"}
    )
    return orchestrator
...
async def test_adapt_interaction_style_integration(processor, learning_orchestrator):
    ...
    feedback = {
        ...
        "interaction_patterns": {"needs_clarity": True, "sensory_sensitivity": True},
        "emotional_responses": {"seeks_support": 0.8},
    }
...
async def test_emotional_memory_persistence(processor, memory_service):
    ...
    similar = await memory_service.get_similar_memories(
        trigger="achievement", context={"user_state": "focused"}
    )
