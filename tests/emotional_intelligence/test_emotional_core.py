...
async def test_learn_from_user(emotional_core, event_bus):
    ...
    emotion = "sympathy"
    context = {"trigger": "friend_struggling", "situation": "academic_challenge"}
...
async def test_develop_relationship(emotional_core, event_bus):
    ...
    for call in event_bus.publish.call_args_list:
        event = call[0][0]
        if (
            event.type == EventType.EMOTIONAL
            and event.data["action"] == "relationship"
            and event.data["event_type"] == "crush_developed"
        ):
            crush_event = event
            break
...
async def test_process_emotion_resilience(emotional_core):
    ...
    trigger = "challenge"
    context = {"situation": "difficult_task", "attitude": "determined"}
...
async def test_emotional_memory_association(emotional_core):
    ...
    await emotional_core.learn_from_user(
        "satisfaction", context1, "It feels good to help others and see them succeed"
    )

    await emotional_core.learn_from_user(
        "joy", context2, "Helping others brings happiness and connection"
    )

    # Process a new similar situation
    state = await emotional_core.process_emotion(
        "helping", {"situation": "volunteer_work"}
    )
