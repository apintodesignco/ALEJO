...
async def test_video_stream(
    ...
    try:
        ...
        logger.info("Camera status: active=%s, fps=%s", 
                  camera_manager.active_camera is not None,
                  camera_manager.actual_fps, flush=True)
        ...
        try:
            ...
            while True:
                ...
                if camera_manager.test_mode and elapsed_time >= test_duration:
                    logger.info(f"Test duration {test_duration}s reached, stopping...", flush=True)
                    break
                ...
                if event_bus:
                    ...
                    logger.info(
                        ...
                        f"Time remaining: {max(0, test_duration - elapsed_time):.1f}s", flush=True
                    )
                    ...
                    if stats_interval >= 5:
                        ...
                        logger.info("- Camera: active=%s, test_mode=%s", 
                                  camera_manager.active_camera is not None,
                                  camera_manager.test_mode, flush=True)
                        logger.info("- Stream manager: running=%s", stream_manager.is_running, flush=True)
                        logger.info("- Frame buffer: size=%d/%d", 
                                  stream_manager.frame_buffer.qsize(),
                                  stream_manager.frame_buffer.maxsize, flush=True)
                        logger.info("- Last frame time: %.2f", stream_manager.last_frame_time, flush=True)
                else:
...
def main():
    ...
    try:
        ...
        try:
            ...
            test_task = loop.create_task(test_video_stream(
...
if __name__ in ["__main__", "alejo.vision.test_video_stream"]:
    ...
    try:
        ...
        print(f"=== Test module exiting with {'success' if success else 'failure'} ===", flush=True)
