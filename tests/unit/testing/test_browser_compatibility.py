...
class TestBrowserCompatibilityTester(unittest.TestCase):
    ...
    def test_create_driver(self, mock_detector, mock_firefox, mock_chrome):
        ...
        # Mock browser detector
        mock_instance = mock_detector.return_value
        mock_instance.get_webdriver_path.side_effect = lambda browser: f"/path/to/{browser}driver"
    ...
    @patch('alejo.testing.browser_testing.BrowserCompatibilityTester.get_available_browsers')
    ...
    def test_compare_test_results(self, mock_get_summary):
        ...
        self.assertEqual(comparison["browsers"]["chrome"]["load_time_diff"], -0.3)  # 1.2 - 1.5
        self.assertEqual(comparison["browsers"]["chrome"]["element_count_diff"]["links"], 1)  # 6 - 5
