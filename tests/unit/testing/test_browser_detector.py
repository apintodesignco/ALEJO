...
class TestBrowserDetector(unittest.TestCase):
    ...
    def test_windows_browser_detection(self, mock_isfile):
        ...
        with patch('platform.system', return_value='Windows'):
            ...
            mock_isfile.side_effect = lambda path: 'chrome.exe' in path or 'firefox.exe' in path
            ...
            self.assertNotIn('edge', browsers)  # Edge should not be detected with our mock
    ...
    def test_macos_browser_detection(self, mock_isfile):
        ...
        with patch('platform.system', return_value='Darwin'):
            # Mock file existence checks
            mock_isfile.side_effect = lambda path: 'Google Chrome' in path or 'Safari' in path
            ...
            self.assertNotIn('firefox', browsers)  # Firefox should not be detected with our mock
    ...
    def test_webdriver_path_detection(self, mock_run):
        ...
        self.assertIsNone(geckodriver_path)  # Should be None since our mock returns failure
    
    def test_browser_info(self):
        """Test getting comprehensive browser information"""
        # Mock methods
        with patch.object(BrowserDetector, 'detect_browsers') as mock_detect, \
             patch.object(BrowserDetector, 'get_browser_version') as mock_version, \
             patch.object(BrowserDetector, 'get_webdriver_path') as mock_webdriver:
            
            mock_detect.return_value = {
                'chrome': '/path/to/chrome',
                'firefox': '/path/to/firefox'
            }
            mock_version.side_effect = lambda browser: "100.0" if browser == 'chrome' else "99.0"
            mock_webdriver.side_effect = lambda browser: "/path/to/chromedriver" if browser == 'chrome' else None
