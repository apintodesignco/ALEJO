...
class DatabaseManager:
    ...
    def store_memory(self, user_id: str, memory_type: str, content: str, 
        ...
        with self._get_connection() as conn:
            ...
            query = '''
                ...
                (user_id, memory_type, content, embedding, importance, created_at, last_accessed) 
                VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            '''
            cursor.execute(query, (user_id, memory_type, content, embedding, importance))
            conn.commit()
            
            # Invalidate related cache entries
            self._invalidate_cache(f"retrieve_memories:{user_id}:{memory_type}")
    ...
    def retrieve_memories(self, user_id: str, memory_type: Optional[str] = None, 
        ...
        cache_key = self._cache_key(
            f"retrieve_memories:{user_id}:{memory_type}", 
            limit=limit, offset=offset, min_importance=min_importance
        )
        ...
        with self._get_connection() as conn:
            ...
            # Build query with parameters for optimization
            ...
            if memory_ids:
                ...
                update_query = f"UPDATE memories SET last_accessed = CURRENT_TIMESTAMP WHERE id IN ({placeholders})"
                cursor.execute(update_query, memory_ids)
                conn.commit()
    ...
    def update_relationship_data(self, user_id: str, relationship_type: str, 
        ...
        with self._get_connection() as conn:
            ...
            '''
            cursor.execute(query, (user_id, relationship_type, trust_level, rapport_level))
            conn.commit()
            
            # Invalidate related cache entries
            self._invalidate_cache(f"get_relationship_data:{user_id}")
    ...
    def get_relationship_data(self, user_id: str, relationship_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        """
        # Try to get from cache first
        cache_key = self._cache_key(f"get_relationship_data:{user_id}", relationship_type=relationship_type)
        cached_result = self._get_cached(cache_key)
        if cached_result is not None:
            return cached_result
    ...
    def set_user_preference(self, user_id: str, preference_key: str, preference_value: str) -> None:
        ...
        with self._get_connection() as conn:
            ...
            # Invalidate related cache entries
            self._invalidate_cache(f"get_user_preferences:{user_id}")
    ...
    def get_user_preferences(self, user_id: str, preference_key: Optional[str] = None) -> Dict[str, str]:
        ...
        """
        # Try to get from cache first
        cache_key = self._cache_key(f"get_user_preferences:{user_id}", preference_key=preference_key)
        cached_result = self._get_cached(cache_key)
        if cached_result is not None:
            return cached_result
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(query, params)
            result = {row['preference_key']: row['preference_value'] for row in cursor.fetchall()}
    ...
    def batch_operation(self, operations: List[Tuple[str, List[Any]]]) -> None:
        ...
        with self._get_connection() as conn:
            ...
            try:
                ...
                # Invalidate all cache since we don't know what was affected
